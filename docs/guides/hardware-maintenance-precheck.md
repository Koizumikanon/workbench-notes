...
本文使用泛化示例。真实主机、网络地址、服务角色和业务负责人应保留在私有 runbook。
...
不要在不了解角色的情况下临时停止服务。写操作应当单独获得授权，并在 private runbook 中记录回滚方案。
...
本文可以公开，因为只描述方法。真实维护文档通常会包含主机名、IP、服务角色、当前任务、用户进程、设备序列号和维护时间，这些应保留在 private repo。
